...
function statusLabel(status) { return { new: 'заявка', confirmed: 'подтверждена', done: 'завершена', cancelled: 'отменена', change: 'запрос переноса' }[status] || status; }
...
export default function ClientPortal({ user, onUserUpdate }) {
  ...
  const requestChange = (event, patch) => {
    if (!window.confirm(`Отправить запрос изменения записи ${event.date} ${event.time}?`)) return;
    saveEvents(events.map((item) => item.id === event.id ? { ...item, ...patch, status: 'change', note: [item.note, patch.note].filter(Boolean).join(' | ') } : item));
  };
  const cancelEvent = (event) => {
    if (!window.confirm(`Отменить запись ${event.date} ${event.time}?`)) return;
    saveEvents(events.map((item) => item.id === event.id ? { ...item, status: 'cancelled', note: [item.note, 'Клиент запросил отмену'].filter(Boolean).join(' | ') } : item));
  };
  ...
  return <div className="grid gap-3">
    ...
    {tab === 'appointments' && <AppointmentsSection upcoming={upcoming} history={history} staff={staff} requestChange={requestChange} cancelEvent={cancelEvent} />}
...
function AppointmentsSection({ upcoming, history, staff, requestChange, cancelEvent }) {
  const [mode, setMode] = useState('upcoming');
  const list = mode === 'upcoming' ? upcoming : history;
  return <Panel eyebrow="личное расписание" title="Мои сеансы">
    <Segment value={mode} setValue={setMode} options={[['upcoming', `Предстоящие · ${upcoming.length}`], ['history', `История · ${history.length}`]]} />
    <div className="mt-3 grid gap-2">{list.length === 0 && <Empty text={mode === 'upcoming' ? 'Предстоящих сеансов нет.' : 'История пока пустая.'} />}{list.map((event) => <ClientEventCard key={event.id} event={event} staff={staff} requestChange={requestChange} cancelEvent={cancelEvent} />)}</div>
  </Panel>;
}
...
function Segment({ value, setValue, options }) { return <div className="grid grid-cols-2 gap-1 rounded-xl bg-white/10 p-1">{options.map(([id, label]) => <button key={id} onClick={() => setValue(id)} className={cx('rounded-lg px-1 py-2 text-[11px] font-black', value === id ? 'bg-blue-600 text-white' : 'text-emerald-50/70')}>{label}</button>)}</div>; }
