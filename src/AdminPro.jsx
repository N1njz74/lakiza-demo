...
function TopTabs({ tab, setTab }) {
  const tabs = [['calendar', 'Кал.'], ['list', 'Список'], ['load', 'Загр.'], ['users', 'Люди']];
  return <div className="sticky top-[86px] z-40 -mx-2 border-y border-white/5 bg-[#06110b]/92 px-2 py-2 backdrop-blur md:top-[104px]"><div className="grid w-full grid-cols-4 gap-1 rounded-[1.1rem] bg-white/8 p-1">{tabs.map(([id, label]) => <button key={id} onClick={() => setTab(id)} className={cx('rounded-xl px-1 py-2.5 text-[11px] font-black', tab === id ? 'bg-lime-200 text-emerald-950' : 'bg-white/10 text-white')}>{label}</button>)}</div></div>;
}
...
function TimeSlot({ hour, item, working, patchEvent, removeEvent }) {
  ...
  return <div className="grid grid-cols-[40px_minmax(0,1fr)] gap-1.5"><span className="pt-3 text-[10px] text-lime-200">{hour}</span><div className="min-w-0 overflow-hidden rounded-lg bg-lime-200 p-2 text-emerald-950 shadow-xl shadow-black/20"><div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0"><b className="block truncate text-sm">{item.client}</b><div className="truncate text-xs opacity-80">{item.service} · {item.duration} мин</div><div className="truncate text-[9px] font-black uppercase tracking-[.1em] opacity-70">{statusLabel(item.status)}</div></div><div className="flex shrink-0 gap-1"><button onClick={() => patchEvent(item.id, { status: item.status === 'confirmed' ? 'done' : 'confirmed' })} className="h-7 w-7 rounded-full bg-emerald-950 text-[10px] font-black text-lime-100">✓</button><button onClick={() => removeEvent(item.id)} className="h-7 w-7 rounded-full bg-red-700 text-[10px] font-black text-white">×</button></div></div></div></div>;
}

function Agenda({ events, staff, patchEvent, removeEvent }) { return <div className="mt-3 grid gap-2">{events.length === 0 && <Empty text="На выбранную дату записей нет." />}{events.map((e) => <EventCard key={e.id} event={e} master={staffName(staff, e.staffId)} patchEvent={patchEvent} removeEvent={removeEvent} />)}</div>; }
function EventCard({ event, master, patchEvent, removeEvent }) { return <div className="max-w-full overflow-hidden rounded-xl bg-white/10 p-3"><b className="text-lime-100">{event.date} · {event.time}</b><div className="mt-1 truncate text-sm text-emerald-50/80">{event.client} · {event.service}</div><div className="truncate text-xs text-emerald-50/50">{master} · {event.phone}</div><div className="mt-3 flex gap-2"><button onClick={() => patchEvent(event.id, { status: 'confirmed' })} className="rounded-full bg-lime-200 px-3 py-2 text-xs font-black text-emerald-950">Подтв.</button><button onClick={() => removeEvent(event.id)} className="rounded-full bg-red-500/20 px-3 py-2 text-xs font-black text-red-100">Удалить</button></div></div>; }
function ListView({ events, staff, filter, setFilter, patchEvent, removeEvent }) { return <Panel title="Список" note="записи"><Filter staff={staff} value={filter} setValue={setFilter} /><div className="mt-3 grid gap-2">{events.map((e) => <EventCard key={e.id} event={e} master={staffName(staff, e.staffId)} patchEvent={patchEvent} removeEvent={removeEvent} />)}</div></Panel>; }
