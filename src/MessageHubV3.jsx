...
function searchBlob(...items) { return norm(items.flat(Infinity).filter(Boolean).join(' ')); }
function queryParts(query) { return norm(query).split(' ').filter(Boolean); }
function matchesSearch(blob, query) {
  const parts = queryParts(query);
  if (!parts.length) return true;
  const text = searchBlob(blob);
  const numeric = digits(query);
  return parts.every((part) => text.includes(part)) || Boolean(numeric && digits(text).includes(numeric));
}
function eventSearch(event) {
  return searchBlob(event.client, event.clientName, event.phone, event.service, event.note, event.privateNotes, event.clientPublicNote, event.date, event.time, event.status, event.weight, event.height, event.requestedDate, event.requestedTime, event.requestedCancel ? 'отмена отменить' : '', event.requestedDate ? 'перенос перенести' : '');
}
function clientEventSearch(events, clientId, phone, name) {
  const p = phoneKey(phone);
  const n = norm(name);
  return searchBlob(events.filter((event) => {
    const eventPhone = phoneKey(event.phone);
    const eventName = norm(event.client || event.clientName);
    return (p && eventPhone === p) || (clientId && event.clientId === clientId) || (n && eventName && eventName === n);
  }).map(eventSearch));
}
function clientSearch(item, events, id) {
  return searchBlob(item.name, item.surname, item.login, item.phone, item.email, item.privateNotes, item.clientPrivateNotes, item.adminNote, item.adminNotes, item.therapistNote, item.therapistNotes, item.clientPublicNote, item.note, item.notes, item.healthNote, item.medicalNote, item.wishes, item.preferences, item.assignedStaffId, clientEventSearch(events, id, item.phone || item.login, [item.name, item.surname].filter(Boolean).join(' ') || item.name || item.login));
}
...
function buildContacts(user, staff, clients, demoClients, events, threads) {
  ...
  if (user.role === 'client') {
    add(map, { id: 'admin', kind: 'admin', name: 'Администратор', subtitle: 'запись, перенос, отмена, оплата', searchText: searchBlob('администратор запись перенос отмена оплата вопрос согласование') });
    add(map, { id: `staff:${assigned}`, kind: 'employee', name: staffName(staff, assigned), subtitle: 'закреплённый массажист', staffId: assigned, searchText: searchBlob(staffName(staff, assigned), 'закрепленный закреплённый массажист специалист самочувствие пожелания подготовка') });
    return [...map.values()];
  }

  if (user.role === 'admin') {
    staff.forEach((item) => add(map, { id: `staff:${item.id}`, kind: 'employee', name: item.name, subtitle: item.title || 'сотрудник', staffId: item.id, phone: item.phone || '', searchText: searchBlob(item.name, item.title, item.phone, item.address, item.note, item.notes, item.privateNotes, 'сотрудник массажист расписание') }));
    clients.forEach((item) => { const id = phoneKey(item.phone) || slug(item.id || item.name); add(map, { id: `client:${id}`, kind: 'client', name: item.name || 'Клиент', subtitle: item.phone || item.email || 'карточка клиента', clientId: id, phone: item.phone || '', staffId: item.assignedStaffId || '', searchText: clientSearch(item, events, id) }); });
    demoClients.forEach((item) => { const id = phoneKey(item.phone || item.login) || slug(item.login || item.name); const name = [item.name, item.surname].filter(Boolean).join(' ') || item.login || 'Клиент'; add(map, { id: `client:${id}`, kind: 'client', name, subtitle: item.phone || item.email || 'зарегистрирован', clientId: id, phone: item.phone || '', staffId: item.assignedStaffId || '', searchText: clientSearch(item, events, id) }); });
    events.forEach((item) => { const id = phoneKey(item.phone) || slug(item.client || item.clientName); add(map, { id: `client:${id}`, kind: 'client', name: item.client || item.clientName || 'Клиент', subtitle: item.phone || `${item.date || ''} ${item.time || ''}`.trim() || 'из записи', clientId: id, phone: item.phone || '', staffId: item.staffId || '', searchText: searchBlob(item.client, item.clientName, item.phone, eventSearch(item), clientEventSearch(events, id, item.phone, item.client || item.clientName)) }); });
    threads.forEach((thread) => {
      const messagesText = searchBlob((thread.messages || []).map((message) => message.text));
      if (thread.clientId) add(map, { id: `client:${thread.clientId}`, kind: 'client', name: thread.clientName || thread.title || 'Клиент', subtitle: thread.staffName ? `закреплён: ${thread.staffName}` : 'из переписки', clientId: thread.clientId, staffId: thread.staffId || '', searchText: searchBlob(thread.clientName, thread.title, thread.subject, thread.staffName, messagesText) });
      if (thread.staffId && !thread.clientId) add(map, { id: `staff:${thread.staffId}`, kind: 'employee', name: thread.staffName || staffName(staff, thread.staffId), subtitle: 'сотрудник', staffId: thread.staffId, searchText: searchBlob(thread.staffName, staffName(staff, thread.staffId), thread.subject, messagesText) });
    });
    return [...map.values()].sort((a, b) => `${a.kind}-${a.name}`.localeCompare(`${b.kind}-${b.name}`, 'ru'));
  }

  add(map, { id: 'admin', kind: 'admin', name: 'Администратор', subtitle: 'служебные вопросы', searchText: searchBlob('администратор служебные вопросы расписание запись') });
  events.filter((item) => item.staffId === staffId).forEach((item) => { const id = phoneKey(item.phone) || slug(item.client || item.clientName); add(map, { id: `client:${id}`, kind: 'client', name: item.client || item.clientName || 'Клиент', subtitle: item.phone || 'мой клиент', clientId: id, phone: item.phone || '', staffId, searchText: searchBlob(item.client, item.clientName, item.phone, eventSearch(item), clientEventSearch(events, id, item.phone, item.client || item.clientName)) }); });
  threads.filter((item) => item.staffId === staffId && item.clientId).forEach((item) => add(map, { id: `client:${item.clientId}`, kind: 'client', name: item.clientName || item.title || 'Клиент', subtitle: 'мой клиент', clientId: item.clientId, staffId, searchText: searchBlob(item.clientName, item.title, item.subject, (item.messages || []).map((message) => message.text)) }));
...
function buildDialogs(threads, contacts, user) {
  ...
  return [...groups.entries()].map(([id, group]) => {
    ...
    const contact = contactMap.get(id) || { id, kind: id.startsWith('staff:') ? 'employee' : 'client', name: lastThread?.clientName || lastThread?.staffName || lastThread?.title || 'Контакт', subtitle: lastThread?.subject || '', searchText: searchBlob(lastThread?.clientName, lastThread?.staffName, lastThread?.title, lastThread?.subject) };
    const status = group.some((thread) => thread.status === 'pending') ? 'pending' : group.every((thread) => thread.status === 'resolved') ? 'resolved' : 'open';
    const dialogSearch = searchBlob(contact.searchText, contact.name, contact.subtitle, group.map((thread) => [thread.title, thread.subject, thread.clientName, thread.staffName, thread.note, thread.notes]), messages.map((message) => [message.name, message.text, message.subject]));
    return { id, contact: { ...contact, searchText: searchBlob(contact.searchText, dialogSearch) }, threads: group, primaryThreadId: lastThread?.id, messages, lastMessage: messages[messages.length - 1], updatedAt: lastThread?.updatedAt || messages[messages.length - 1]?.at || '', status, searchText: dialogSearch };
...
export default function MessageHubV3({ user }) {
  ...
  const filteredContacts = contacts.filter((item) => matchesSearch(searchBlob(item.searchText, item.name, item.subtitle, item.phone, item.clientId, item.staffId), query));
  const filteredDialogs = dialogs.filter((item) => matchesSearch(searchBlob(item.searchText, item.contact.searchText, item.contact.name, item.contact.subtitle, item.lastMessage?.text), query));
    ...
    {open && <div className="fixed bottom-20 right-2 z-[96] flex h-[84vh] w-[calc(100vw-1rem)] max-w-6xl flex-col overflow-hidden rounded-[1.6rem] border border-white/10 bg-[#05110b]/96 text-white shadow-2xl shadow-black/60 backdrop-blur-xl md:bottom-6 md:right-24 md:h-[790px] md:max-h-[calc(100vh-3rem)]">
      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[390px_1fr]">
        <aside className={cx('min-h-0 overflow-y-auto border-b border-white/10 p-3 md:block md:border-b-0 md:border-r', mobile === 'chat' ? 'hidden' : 'block')}>
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск: имя, фамилия, телефон, заметки, услуги" className="mt-3 w-full rounded-2xl bg-white px-4 py-3 text-sm font-bold text-emerald-950 outline-none placeholder:text-emerald-950/35" />
          {query && <div className="mt-2 rounded-xl bg-white/5 px-3 py-2 text-[10px] font-bold text-emerald-50/45">Поиск учитывает ФИО, телефон, услуги, заметки администратора, заметки массажиста и комментарии в сеансах.</div>}
          ...
          {mode === 'compose' && <Compose contacts={filteredContacts} state={compose} setState={setCompose} send={sendCompose} events={currentEvents} user={user} query={query} />}
        </aside>
